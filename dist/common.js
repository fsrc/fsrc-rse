...
(function(){
  var ref$, any, filter, each, tail, initial, map, lines, groupBy, keys, maximumBy, drop, flatten, spawn2, exec, inspect, fsrcConfig, say, config, MOSH, SSH, SM, SMLIST, SHELL, SC, DMENU, DMENUPROMPTARG, DMENUARGS, TERMINAL, TERMTITLE, TERMEXEC, TERMARGS, spawn, dmenuCmd, createCmd, openCmd, listCmd, sessions, windowNameToId, dmenu, focused, windows, nextIndex, createNew, open, out$ = typeof exports != 'undefined' && exports || this;
  ref$ = require('prelude-ls'), any = ref$.any, filter = ref$.filter, each = ref$.each, tail = ref$.tail, initial = ref$.initial, map = ref$.map, lines = ref$.lines, groupBy = ref$.groupBy, keys = ref$.keys, maximumBy = ref$.maximumBy, drop = ref$.drop, flatten = ref$.flatten;
  ...
  DMENUPROMPTARG = config.dmenu.promptArg;
  DMENUARGS = config.dmenu.args;
  ...
  out$.dmenuCmd = dmenuCmd = function(prompt){
    return DMENUARGS.concat([DMENUPROMPTARG, "'" + prompt + "'"]);
  };
  if (SM !== 'dtach') {
    ...
    out$.createCmd = createCmd = function(termArgs, client, host, session, index){
      ...
      title = host + "" + SC + name;
      return {
        mosh: termArgs.concat([TERMTITLE, title, TERMEXEC, MOSH, host, "--", "dtach", "-c", "~/.dtach/" + name, SHELL]),
        ssh: termArgs.concat([TERMTITLE, title, TERMEXEC, SSH, host, "-t", "--", "dtach", "-c", "~/.dtach/" + name, SHELL])
      }[client];
    };
    out$.openCmd = openCmd = function(termArgs, client, host, session, index){
      var name, title;
      ...
      title = host + "" + SC + name;
      return {
        mosh: termArgs.concat([TERMTITLE, title, TERMEXEC, MOSH, host, "--", "dtach", "-a", "~/.dtach/" + name]),
        ssh: termArgs.concat([TERMTITLE, title, TERMEXEC, SSH, host, "-t", "--", "dtach", "-a", "~/.dtach/" + name])
      }[client];
    };
    out$.listCmd = listCmd = function(host){
      return SSH + " " + host + " 'ls ~/.dtach'";
    };
    out$.sessions = sessions = function(host, callback){
      return exec(listCmd(host), function(err, stdout, stderr){
        if (err != null) {
          ...
          map(function(line){
            var ref$, name, index;
            ref$ = line.split(SC), name = ref$[0], index = ref$[1];
            return {
              name: name,
              index: parseInt(index)
            };
  ...
  out$.dmenu = dmenu = function(prompt, list, callback){
    var cp;
    cp = spawn2(DMENU, dmenuCmd(prompt));
